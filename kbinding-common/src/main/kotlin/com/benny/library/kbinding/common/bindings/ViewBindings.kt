...
fun Drawable.level(): Consumer<Int> = Consumer { level = it }
...
fun View.background(): Consumer<Drawable> = Consumer { bg = it }

fun View.backgroundColor(): Consumer<Int> = Consumer { setBackgroundColor(it) }
...
fun View.until(vararg paths: String, action: () -> Unit): PropertyBinding = oneWayPropertyBinding(paths, Consumer { action() }, true, EmptyOneWayConverter1())
fun <T> View.until(vararg paths: String, converter: OneWayConverter<*, out T>, action: (T) -> Unit): PropertyBinding = oneWayPropertyBinding(paths, Consumer<T> { action(it) }, true, converter)
