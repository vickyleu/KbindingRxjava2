...
class MockSubscription : Disposable {
    /**
...
open class ViewModel() : IViewModel {
    ...
    private fun <T> addDependsOn(key: String, dependsOn: Array<out String>, getter: () -> T) = this.dependsOn.put(key,
            oneWayPropertyBinding(dependsOn, Consumer<T> { t -> property<T>(key).value = t }, false, OneWayConverter<Any?, T> { getter() }))

    private fun bindDependsOn(key: String): Disposable {
    ...
    fun <T> bindCommand(key: String, cmdAction: (T, (Boolean) -> Unit) -> Unit): ReadOnlyProperty<Any, Command<T>> {
        addCommand(key, Command<T> { t, action -> cmdAction(t, { action.accept(it) /*action.call(it)*/ }) })
    ...
    override fun <T, R> bind(oneWayPropertyBinding: OneWayPropertyBinding<T, R>): Disposable {
    ...
    override fun <T> bind(multiplePropertyBinding: MultiplePropertyBinding<T>): Disposable {
        return CompositeDisposable().apply {
    ...
    override fun <T, R> bind(twoWayPropertyBinding: TwoWayPropertyBinding<T, R>): Disposable {
        return twoWayPropertyBinding.bindTo(property(twoWayPropertyBinding.key))
    }

    override fun <T> bind(commandBinding: CommandBinding<T>): Disposable {
