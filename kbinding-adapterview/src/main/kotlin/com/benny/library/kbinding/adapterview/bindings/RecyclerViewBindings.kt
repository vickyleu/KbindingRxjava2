...
fun RecyclerView.setOnItemClickListener(onItemClickListener: AdapterView.OnItemClickListener?) {
    ...
    (tag as HashMap<String, Any?>)[ON_ITEM_CLICK_LISTENER] = onItemClickListener
...
fun RecyclerView.setPagingListener(pagingListener: AdapterPagingListener<*>?) {
    if (tag !is HashMap<*, *>) tag = HashMap<String, Any?>()
    (tag as HashMap<String, Any?>)[PAGING_LISTENER] = pagingListener
...
@Suppress("UNCHECKED_CAST")
fun RecyclerView.swapAdapter(): Consumer<RecyclerView.Adapter<RecyclerView.ViewHolder>> {
    return Consumer { it ->
...
fun RecyclerView.paging(path: String): PropertyBinding = commandBinding(path, paging(), Consumer { (this.adapter as? AdapterPagingCompleteListener)?.onPagingComplete(!it) })
fun RecyclerView.itemClick(path: String): PropertyBinding = commandBinding(path, itemClicks(), Consumer {})
//Property
